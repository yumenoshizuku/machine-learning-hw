...
def scan(filename, exclude_stopwords = False, binary_label = False):
    ...
    with open(filename, 'r') as f:
        while True:
            ...
            if binary_label:
                # copy positive and negative reviews into respective lists
                score = score_to_binary(score)
		if score == 1:
		    data_positive.append([review, score])
		elif score == 0:
		    data_negative.append([review, score])
            datum = [review, score]
            data.append(datum)
    return data, data_positive, data_negative
